...
func (w *Worker) processTask(task *models.Task) {
	...
	if err != nil {
		...
		w.updateTaskStatus(task, models.TaskStatusError)
		return
	}
	...
	w.updateTaskStatus(task, models.TaskStatusDone)
}

func (w *Worker) updateTaskStatus(task *models.Task, status models.TaskStatus) {
	err := w.taskRepo.Update(context.Background(), task.ID, models.TaskUpdateInput{Status: status})
	if err != nil {
